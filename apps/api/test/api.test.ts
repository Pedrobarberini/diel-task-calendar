...
test('task CRUD normalizes timezone and replaces tags without losing creation timestamp', async (t) => {
  ...
  const updated = await app.inject({
    ...
    payload: taskBody({ title: 'Entrega final', tagIds: [] }),
  });
  ...
  assert.equal(
    (await app.inject({ method: 'DELETE', url: `/api/tasks/${created.id}` })).statusCode,
    204,
  );
...
test('tag rename and deletion update relationships while preserving tasks', async (t) => {
  ...
  const renamed = await app.inject({
    method: 'PUT',
    url: `/api/tags/${tag.id}`,
    payload: { name: 'Equipe', color: '#123456' },
  });
  assert.equal(renamed.statusCode, 200);
  assert.equal((await app.inject('/api/tasks')).json().data[0].tags[0].name, 'Equipe');
  assert.equal(
    (await app.inject({ method: 'DELETE', url: `/api/tags/${tag.id}` })).statusCode,
    204,
  );
...
test('duplicate tag names are rejected with case-insensitive Unicode normalization', async (t) => {
  ...
  const duplicate = await app.inject({
    method: 'POST',
    url: '/api/tags',
    payload: { name: '  REUNIÃO  ', color: '#112233' },
  });
  assert.equal(duplicate.statusCode, 409);
  assert.equal(duplicate.json().error.code, 'TAG_NAME_EXISTS');
  const second = await createTag(app, 'Estudos');
  const duplicateEdit = await app.inject({
    method: 'PUT',
    url: `/api/tags/${second.id}`,
    payload: { name: first.name, color: '#112233' },
  });
...
test('interval filtering includes cross-midnight overlaps and excludes touching boundaries', async (t) => {
  ...
  const response = await app.inject(
    listUrl({ from: '2026-09-22T00:00:00Z', to: '2026-09-23T00:00:00Z' }),
  );
  assert.deepEqual(
    response.json().data.map((task: Task) => task.id),
    [crossing.id, inside.id],
  );
});
...
test('multiple tag filters use OR semantics, combining with literal title-only search', async (t) => {
  ...
  assert.deepEqual(
    new Set(filtered.json().data.map((task: Task) => task.id)),
...
test('validation rejects invalid durations, dates, repeated tags and unknown fields', async (t) => {
  ...
  assert.equal(
    (await app.inject(listUrl({ from: '2026-09-23T00:00:00Z', to: '2026-09-22T00:00:00Z' })))
      .statusCode,
    400,
  );
  ...
  assert.equal(
    (await app.inject({ method: 'DELETE', url: '/api/tasks/not-a-uuid' })).statusCode,
    400,
  );
});
...
test('missing resources and malformed requests use the structured error contract', async (t) => {
  ...
  const oversized = await app.inject({
    method: 'POST',
    url: '/api/tasks',
    payload: taskBody({ description: 'x'.repeat(70000) }),
  });
...
test('API provides health, security headers and configured CORS origin', async (t) => {
  ...
  const response = await app.inject({
    url: '/api/health',
    headers: { origin: 'https://calendar.example' },
  });
...
test('holidays include only national public dates and cache expires after its TTL', async (t) => {
  ...
  const app = await setup(t, {
    now: () => now,
    holidayCacheTtlMs: 100,
    holidayFetcher: async (url, init) => {
      calls++;
