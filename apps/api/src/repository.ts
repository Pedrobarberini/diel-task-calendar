...
export class Repository {
  ...
  listTags(): Tag[] {
    return this.db
      .prepare('SELECT id, name, color FROM tags ORDER BY name_key')
  ...
  private requireTag(id: string): Tag {
    const tag = this.db
      ...
      .get(id) as unknown as Tag | undefined;
  ...
  saveTag(input: TagInput, id?: string): Tag {
    return transaction(this.db, () => {
      ...
      if (id) {
        this.db
          .prepare('UPDATE tags SET name = ?, name_key = ?, color = ? WHERE id = ?')
          .run(input.name, nameKey, input.color, id);
      } else {
        this.db
          .prepare('INSERT INTO tags (id, name, name_key, color) VALUES (?, ?, ?, ?)')
  ...
  listTasks(query: TaskQuery): Task[] {
    ...
    if (query.tagIds.length) {
      conditions.push(
        `EXISTS (SELECT 1 FROM task_tags WHERE task_id = tasks.id AND tag_id IN (${query.tagIds.map(() => '?').join(',')}))`,
      );
  ...
  getTask(id: string): Task {
    ...
    const tags = this.db
      ...
      JOIN task_tags tt ON tt.tag_id = t.id WHERE tt.task_id = ? ORDER BY t.name_key`,
      )
    ...
    return transaction(this.db, () => {
      ...
      for (const tagId of input.tagIds) {
        if (!this.db.prepare('SELECT id FROM tags WHERE id = ?').get(tagId)) {
          throw new ApiError(400, 'UNKNOWN_TAG', 'Uma das etiquetas selecionadas não existe.', {
            tagId,
          });
        }
      }
      ...
      if (id) {
        this.db
          .prepare(
            `UPDATE tasks SET title = ?, description = ?, starts_at = ?, starts_at_ms = ?,
