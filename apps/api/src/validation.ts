...
export const taskSchema = z.object({
  ...
  tagIds: z.array(idSchema).max(20).default([])
    .refine((ids) => new Set(ids).size === ids.length, 'Não repita uma etiqueta na mesma tarefa.'),
}).strict();

export const tagSchema = z.object({
  name: z.string().trim().min(1).max(40),
  color: z.string().regex(/^#[0-9a-fA-F]{6}$/, 'Use uma cor hexadecimal, como #2563eb.')
...
export const taskQuerySchema = z.object({
  ...
  tagIds: z.string().max(739).optional().transform((value) => value ? value.split(',') : [])
